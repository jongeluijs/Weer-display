...
export function createRouter({
  ...
  async function handle(req, res) {
    ...
    {
      const m = p.match(/^\/api\/floorplan\/device\/([^/]+)\/usage$/);
      if (m) {
        if (!floorplan || !shellyHistory) {
          sendJson(res, 503, { ok: false, error: 'shelly history disabled' });
          return;
        }
        const id = decodeURIComponent(m[1]);
        const dev = floorplan.getById(id);
        ...
        // Live snapshot voor het huidige uur en de "now"-regel.
        let now = null;
        try {
          const status = await deviceControl.getStatus(dev);
          now = status?.online
            ...
            : { online: false, error: status?.error || null };
        } catch (err) {
          now = { online: false, error: err.message };
        }
        const payload = { ok: true, range, device: dev, now };
        if (range === 'today') {
          payload.hourly = await shellyHistory.getHourlyMeter(
            id,
            null,
            now?.total_kwh ?? null
          );
          payload.today = await shellyHistory.getToday(id);
        } else if (range === '7d' || range === '30d') {
          const days = range === '7d' ? 7 : 30;
          payload.daily = await shellyHistory.getDaily(id, days);
        } else {
          sendJson(res, 400, { ok: false, error: 'invalid range' });
          return;
        }
