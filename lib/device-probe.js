...
async function probeShelly(ip) {
  try {
    ...
    if (gen >= 2) {
      const isPmOnly = /pm.*mini|minipm/i.test(String(data.app || ''));
      return {
        vendor: 'Shelly',
        type: isPmOnly ? 'plug' : 'switch',
        friendlyName: data.name || data.app || data.id || 'Shelly',
        model: data.model || null,
        details: { app: data.app, gen, fw: data.ver, id: data.id, mac: data.mac },
        capabilities: isPmOnly ? ['meter'] : ['toggle', 'on', 'off', 'meter'],
        driver: 'shelly_gen2',
      };
    }
    return {
